<script lang="ts">
  ...
  function setDefaultBang(value: string) {
    if (!input.startsWith("!") || input.length === 1 || input.includes(" ")) {
...
<div
  class="flex flex-col flex-wrap gap-y-5 m-auto text-left w-[95%] sm:w-2xl p-3"
>
  <section>
    <h1 class="text-2xl">theme</h1>
    <div class="p-3">
      <a class="hover:underline hover:text-accent" href="/themes"
        >navigate to the themes page &gt;</a
      >
    </div>
  </section>
    ...
    <div class="flex p-3 gap-x-4">
      <label class="flex items-center cursor-pointer">
        <input
          ...
          class="cursor-pointer hidden"
        />
        <span class:underline={$useSuggestions === true}>duckduckgo</span>
      </label>
      <label class="flex items-center cursor-pointer">
        <input
          type="radio"
  ...
  <section>
    ...
    <div class="p-3 flex flex-wrap flex-row">
      ...
      <div class="flex gap-x-2">
        <button onclick={() => setDefaultBang(input)} class="p-2 cursor-pointer"
          >Save</button
        >
        <button
          onclick={() => {
